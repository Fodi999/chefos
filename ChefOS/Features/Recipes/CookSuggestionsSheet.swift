...
struct CookSuggestionsSheet: View {
                ...
                if vm.isLoading {
                    ...
                    ScrollView {
                        VStack(spacing: 20) {
                            ...
                            // Unlock Suggestions
                            if let unlock = vm.unlockSuggestions,
                               !unlock.unlockHints.isEmpty {
                                unlockCard(unlock)
                            }
                        }
                        .padding()
        ...
        VStack(spacing: 10) {
            HStack(spacing: 16) {
                insightMetric(
                    ...
                    value: "\(insight.totalIngredients)",
                    label: l10n.t("cook.ingredients")
                )
                insightMetric(
                    icon: "calendar.badge.clock",
                    value: "~\(insight.daysLeft)",
                    label: l10n.t("cook.daysLeft")
                )
                insightMetric(
                    icon: "exclamationmark.triangle.fill",
                    value: "\(insight.wasteRisk)%",
                    label: l10n.t("cook.wasteRisk"),
                    color: insight.wasteRisk > 30 ? .red : .orange
                )
            }
            ...
            if !insight.atRisk.isEmpty {
                HStack(spacing: 4) {
                    Image(systemName: "clock.badge.exclamationmark")
                        .font(.caption2)
                        .foregroundStyle(.orange)
                    Text(insight.atRisk.joined(separator: ", "))
                        .font(.caption2)
                        .foregroundStyle(.orange)
                        .lineLimit(2)
                }
            ...
            if !dish.steps.isEmpty {
                HStack(spacing: 4) {
                    Image(systemName: "list.number")
                        .font(.caption2)
                        .foregroundStyle(.secondary)
                    Text("\(dish.steps.count) \(l10n.t("cook.steps"))")
                        .font(.caption2)
                        .foregroundStyle(.secondary)
                    if let totalTime = dish.steps.compactMap(\.timeMin).reduce(0, +) as Int?,
                       totalTime > 0 {
                        Text("• ~\(totalTime) \(l10n.t("cook.min"))")
                            .font(.caption2)
                            .foregroundStyle(.secondary)
                    }
                }
    ...
    private func dishTypeBadge(_ type: String) -> some View {
        let icon: String = {
            ...
            case "stir_fry", "stirfry": return "frying.pan.fill"
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    if !dish.insight.reasons.isEmpty {
                        reasonsSection
                    }
                    ...
                    if let adaptation = dish.adaptation, adaptation.changed {
                    ...
                    if !dish.warnings.isEmpty {
                        warningsSection
                    }

                    if !dish.tags.isEmpty || !dish.allergens.isEmpty {
                        tagsSection
                    }
                }
                .padding()
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 12) {
                Label(dish.dishType.capitalized, systemImage: "fork.knife")
                    .font(.caption)
                Label(dish.complexity.capitalized, systemImage: "speedometer")
                    .font(.caption)
        ...
        sectionCard(title: l10n.t("cook.flavorProfile"), icon: "sparkles", color: .purple) {
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    ...
                    .frame(height: 8)
                    Text(String(format: "%.0f%%", flavor.balanceScore * 100))
                        .font(.caption.weight(.bold))
                }
                        ...
                        ForEach(flavor.suggestions, id: \.self) { s in
                            HStack(spacing: 4) {
                                Image(systemName: "plus.circle")
                                    .font(.caption2)
                                    .foregroundStyle(.orange)
                                Text(localizeFlavorSuggestion(s))
                                    .font(.caption)
                            }
                        }
                    }
            ...
            VStack(spacing: 6) {
                ForEach(dish.ingredients, id: \.slug) { ing in
                    HStack {
                        Circle()
                            .fill(ing.available
                                  ? (ing.expiringSoon ? Color.orange : Color.green)
                                  : Color.red.opacity(0.5))
                            .frame(width: 8, height: 8)
                        Text(ing.name).font(.subheadline)
                        Spacer()
                        Text(String(format: "%.0fg", ing.grossG))
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            HStack(spacing: 8) {
                                if let time = step.timeMin {
                                    Label("\(time) \(l10n.t("cook.min"))", systemImage: "clock")
